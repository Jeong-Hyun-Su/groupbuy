...
| 항목 | 내용 |
|---|---|
| 문서 버전 | v0.2 (Phase 1 구현·점검 반영 — 변경 내역은 [부록 C](#부록-c-v02-변경-내역)) |
| 작성일 | 2026-09-15 (v0.2: 2026-09-21) |
...
stateDiagram-v2
    ...
    OPEN --> CLOSING : 판매자 사유 중단 (강제 무산)
    SUCCEEDED --> SETTLED : 정산 생성 (일 배치)
    FAILED --> [*] : 전액 환불 완료
    SETTLED --> [*]
    CANCELLED --> [*]
```
...
| FAILED | ✗ | 무산. 전액 환불 진행 |
| SETTLED | ✗ | 정산 생성 완료. **환불 완료를 기다리지 않는다** — 판매자 지급액은 `final_amount` 로 이미 확정이라, 환불 1건이 MANUAL_REQUIRED 에 걸려도 판매자 정산 전체가 막히면 안 된다. 미완 환불은 운영 큐(UC-14)와 원장 대조가 추적한다 |
| CANCELLED | ✗ | 오픈 전 판매자 취소 |

OPEN 중 판매자 사유(품절 등)로 중단해야 하면 새 상태를 만들지 않고 마감 경로를 재사용한다: `OPEN → CLOSING → FAILED`(강제 무산) → 전원 전액 환불.
...
**선점 만료 시각 = `min(선점 시각 + TTL, 딜 마감 시각)`.** TTL 을 그대로 주면 마감 9분 전 선점자는 마감 뒤에 결제하게 되고, 그 결제는 전부 "승인 → 전액 취소"([ADR-07](#adr-07-마감-시점의-선점-처리))가 된다. 만료를 마감에 맞춰 자르고 **PG 승인 호출 전에 만료·딜 상태를 검사**하면 대부분은 승인 자체가 일어나지 않는다.
...
5. 클라이언트가 성공 리다이렉트 후 confirm 을 호출한다. 시스템은 선점 만료·딜 상태를 검사한 뒤 PG 승인 API 를 호출한다 (토스는 가맹점이 승인 API 를 불러야 결제가 완료된다)
6. 시스템은 승인 결과를 기록하고 참여를 CONFIRMED로 전이한다
...
- 5a. 결제 승인 API 호출 실패 → **PG 조회로 실제 상태를 먼저 확인**(타임아웃이어도 승인됐을 수 있다). 미승인이 확실할 때만 실패 처리 + 사용자 안내, 선점은 TTL 로 해제
- 5c. 승인 직후 프로세스 사망 → PG 웹훅이 도착하면 PG 조회 결과를 반영해 복구
- 5b. 승인 직전 딜이 마감됨·선점 만료 → **결제는 APPROVED 로 기록한 뒤** 전액 취소, 참여는 확정하지 않음. 승인 기록을 남기지 않으면 환불할 근거가 없다 ([ADR-07](#adr-07-마감-시점의-선점-처리))
...
> 목표치는 방향이다. 로컬·EC2 1~2대에서 10,000 TPS 에 못 미쳐도 실패가 아니다. "N TPS 에서 X 가 먼저 무너졌고, 조치 후 M 이 됐다"는 기록(13.2)이 산출물이다.

### 4.2 정합성 목표

| 항목 | 목표 |
|---|---|
| 정원 초과 (R1) | 0건 — 부하테스트 후 `COUNT(confirmed) <= capacity` 검증. Redis 유실 시엔 DB 방어선이 없으므로 **탐지·보상**으로 지킨다 (7.4) |
| 승인됐는데 주인 없는 돈 (R5) | 0건 — `APPROVED 결제 중 참여가 미확정이고 환불도 없는 건` 조회 |
...
| idempotency_key | `refund-{payment_id}-{reason}`. PG 호출 시 Idempotency-Key 헤더로 전달. `UNIQUE`. `refund_id` 를 쓰지 않는 이유: id 는 insert 후에야 나오므로, 트랜잭션이 롤백된 뒤의 재시도가 같은 키를 다시 만들 수 없다. (결제, 사유)는 자연 키다 — 자진 취소 후 재참여는 주문·결제가 새로 생기므로 충돌하지 않는다 |
...
| 키 | 타입 | 값 | TTL | 용도 |
|---|---|---|---|---|
| `deal:{dealId}:count` | String(int) | 선점+확정 인원 합 | 마감 + 1일 | **정원 검사 전용** |
| `deal:{dealId}:confirmed` | String(int) | 확정 인원 | 마감 + 1일 | **게이지·예상 할인율**. 확정 할인율은 CONFIRMED 만 세므로(ADR-07), 선점을 섞어 보여주면 "30% 달성" 이 떴다가 20% 로 확정되는 사고가 난다 |
...
-- 마감 시각 ~ 스케줄러 폴링(≤5초) 사이에도 status 는 아직 OPEN 이다. 시각으로 직접 막는다
if tonumber(ARGV[3]) >= tonumber(redis.call('HGET', KEYS[4], 'close_at')) then return {-3, 0} end
...
결제 확정 시에는 `reservations` ZSet에서 제거하고 `confirmed` 를 INCR 한다(`members`·`count`는 유지). 확정자의 자진 취소는 `count`·`confirmed` 둘 다 DECR.
...
| Redis 유실·failover 로 정원 초과 선점 발생 | R2 는 DB UNIQUE 가 막지만 **R1 은 DB 방어선이 없다**(AOF everysec 은 최대 1초 유실). 막는 대신 탐지·보상한다: 마감 판정 시 `confirmed > capacity` 면 `confirmed_at` 후순위 초과분을 전액 환불하고 정원만큼만 확정. 발생 건수는 메트릭·알림 |
...
**릴레이 구현 주의**
- "마지막으로 보낸 id 보다 큰 것" 커서로 읽지 않는다. id(시퀀스) 순서는 커밋 순서가 아니다 — id 100 이 id 101 보다 늦게 커밋되면 커서는 100 을 영영 건너뛴다. `WHERE published_at IS NULL ORDER BY id` 로 읽는다
- 릴레이는 한 번에 하나만 돈다(`FOR UPDATE SKIP LOCKED` 배치 점유). 여럿이 동시에 발행하면 같은 딜의 이벤트 순서가 뒤집힌다
- 그래도 순서는 "대체로" 만 보장된다. 실시간 컨슈머는 payload 의 `currentCount` 를 그대로 내보내지 말고 **Redis 현재값을 읽어** 발행한다 — 이벤트는 "변했다"는 신호로만 쓴다. 게이지가 역행하지 않는다

### 8.4 컨슈머 멱등성

모든 컨슈머는 `processed_events(consumer_group, event_id)` 테이블에 처리 기록을 남기고, 이미 있으면 skip. **처리 기록 insert 와 비즈니스 변경은 같은 DB 트랜잭션**이어야 한다 — 따로 커밋하면 그 사이에 죽었을 때 "처리했다고 적혔는데 안 한" 이벤트가 생긴다. 환불 워커는 이에 더해 `refunds.idempotency_key`로 PG 레벨 멱등성까지 이중 보장.
...
| POST | `/api/payments/webhook` | PG 웹훅. **승인하지 않는다.** 토스의 DONE 웹훅은 가맹점의 승인 호출 이후에만 오고, 결제 웹훅엔 서명이 없다. 본문의 `orderId` 로 PG 를 조회해 DONE 이면 반영 — 승인 직후 프로세스가 죽은 경우의 복구 경로이자, 위조 웹훅이 무해해지는 검증 방식 |
| DELETE | `/api/participations/{id}` | 자진 취소 (딜 OPEN 중, **마감 1시간 전까지** — [ADR-11](#adr-11-자진-취소-마감선)). 전액 환불 생성 |
...
`currentCount`는 **확정 인원**이고 예상 할인율의 기준이다. `occupiedCount`(선점 포함)는 "남은 자리" 표시에만 쓴다. 둘 다 Redis에서 읽는다. DB를 조회하지 않아야 상세 페이지가 트래픽을 견딘다.
...
sequenceDiagram
    ...
    alt 중복 / 정원초과 / 미오픈
        ...
        API->>DB: 사전 검사 (금액, 선점 만료, 딜 OPEN) — 실패면 승인하지 않음
        API->>PG: 결제 승인 API (paymentKey, orderNo, amount)
        PG-->>API: 승인 결과 (실패·타임아웃이면 PG 조회로 재확인)
        API->>DB: TX: 딜 행 잠금, payment(APPROVED), participation(CONFIRMED), outbox(participation.confirmed)
        Note over API,DB: 그 사이 만료·마감됐으면 payment(APPROVED)만 커밋 → 전액 취소 (ADR-07)
        PG-)API: POST /payments/webhook (승인 이후 도착)
        API->>PG: 결제 조회 → 이미 반영됐으면 skip, 아니면 반영 (복구)
...
sequenceDiagram
    ...
    loop 환불 워커 (병렬 N개)
        ...
        W->>PG: 부분취소 API (paymentKey, amount, Idempotency-Key=refund-{paymentId}-{reason})
        alt 성공
    ...
    Note over S,DB: SETTLED 전이는 정산 배치(10.5)가 한다. 환불 완료를 기다리지 않는다
```

**마감 점유의 회수**: CLOSING 점유 후 판정 전에 죽은 딜은 아무도 다시 집지 않는다. 마감 대상 조회는 `OPEN` 뿐 아니라 `CLOSING` 이면서 `closing_started_at` 이 타임아웃(예: 5분)을 넘긴 딜도 포함한다. 판정은 딜 행 잠금 + 상태 전이로 한 번만 일어난다. (Phase 1 은 단일 인스턴스라 타임아웃 없이 CLOSING 을 바로 재개한다)
...
| 결제 | 토스 테스트 결제 승인, 웹훅(PG 조회 후 반영), 승인 후 확정 불가 건 전액 환불 |
| 인증·문서 | Spring Security + JWT, OpenAPI(springdoc), JaCoCo — 완료 기준 "커버리지 80%" 의 증거 |
| 마감 | 단순 스케줄러(5초 폴링), 단일 인스턴스 가정. 롤백된 마감은 CLOSING 에서 재개 |
...
- **보완 (v0.2)**: 점유(CLOSING)와 판정이 다른 트랜잭션이라, 판정이 롤백되거나 워커가 죽으면 딜이 CLOSING 에 갇힌다. 폴링이 오래된 CLOSING 도 집어 재개한다 (10.3)
...
- **트레이드오프**: 마감 직전 참여자는 결제했는데 취소될 수 있음 → 선점 만료를 마감 시각으로 자르고(2.5), PG 승인 전에 만료·딜 상태를 검사해 **승인 자체를 막는다**. 검사와 승인 사이를 빠져나간 건만 "승인 기록 → 전액 취소" 로 간다. 결제창에는 "마감 N초 전" 경고
- **구현 주의**: 확정 불가를 예외로 처리하면 승인 기록 트랜잭션이 롤백돼 PG 는 돈을 받았는데 DB 엔 결제 행이 없다. 결과값으로 돌려주고 결제는 APPROVED 로 커밋한 뒤 환불한다. 확정과 마감 점유는 같은 딜 행 잠금으로 직렬화한다
...
### ADR-11 자진 취소 마감선
- **문제**: OPEN 중 자유 취소를 허용하면 인원을 부풀려 티어를 달성시킨 뒤(다른 사람의 참여를 유도) 마감 직전에 빠질 수 있다
- **선택지**: (a) 제한 없음 (b) 마감 N시간 전부터 취소 불가 (c) 취소 수수료
- **결정**: (b), 기본 1시간
- **근거**: (c)는 결제·정산 모델을 복잡하게 만들고 PG 부분취소 금액 계산이 하나 더 생긴다. (b)는 규칙 한 줄이고, 마감 직전 게이지가 "진짜 인원"이 된다
- **트레이드오프**: 마감 1시간 안에 마음이 바뀐 사용자는 취소 불가 → 참여 화면에 명시
...
| 리스크 | 영향 | 대응 |
|---|---|---|
| 범위 과다로 미완성 | 전체 무산 | Phase 1~3까지를 "최소 포트폴리오"로 정의. 4·5는 별도 마일스톤. 각 Phase 끝에 배포 가능 상태 유지. **시간이 모자라면 자르는 순서**: Elasticsearch(도메인 필연성이 가장 약하다 — PostgreSQL 검색으로 충분하고 "왜 안 썼는가" ADR 도 유효) → AWS(로컬 k3s 로 대체) → Debezium. Kafka·Redis·k8s·관측성은 지킨다 |
| 토스 연동 가정이 실제와 다름 (v0.1 은 "웹훅이 승인을 대신할 수 있다", "결제 웹훅에 서명이 있다"고 가정했고 둘 다 틀렸다) | 돈이 새는 경로 | 외부 연동은 설계 전에 공식 문서로 확인하고 근거 링크를 ADR 에 남긴다 |
...
진행 상태는 저장소 `README.md` 의 Phase 체크리스트가 정본이다.

## 부록 C. v0.2 변경 내역

Phase 1 구현을 설계와 대조하며 찾은 것들. 앞의 셋은 코드 버그로도 존재했고 수정·테스트했다.

| # | 문제 | 반영 위치 |
|---|---|---|
| 1 | 판정이 롤백되면 딜이 CLOSING 에 영구 고착 (마감 대상 조회가 OPEN 만 봄) | 10.3, ADR-05 |
| 2 | PG 승인 후 참여 확정이 예외로 실패하면 승인 기록이 롤백 — 돈만 받고 끝 | UC-04 5b, 10.1, ADR-07 |
| 3 | 토스 웹훅 가정 오류: DONE 웹훅은 승인 이후에만 옴, 결제 웹훅엔 서명 없음 | UC-04, 9.2, 10.1, 14 |
| 4 | 선점 TTL 이 마감을 넘어감 | 2.5 |
| 5 | 게이지(선점+확정)와 확정 할인율(확정만)의 기준 불일치 | 7.1, 7.3, 9.3 |
| 6 | Lua 가 `close_at` 을 검사하지 않음 (ARGV[3] 미사용) | 7.2 |
| 7 | 자진 취소를 이용한 티어 조작 | 9.2, ADR-11 |
| 8 | Outbox 릴레이의 id 커서 누락·순서 역전, 컨슈머 멱등 기록의 트랜잭션 경계 | 8.3, 8.4 |
| 9 | SETTLED 의미 충돌, 환불 1건이 판매자 정산 전체를 막음 | 2.4, 10.3, 10.5 |
| 10 | Redis 유실 시 R1 의 DB 방어선 부재 | 4.2, 7.4 |
| 11 | OPEN 중 판매자 사유 중단 경로 없음 | 2.4 |
| 12 | 환불 멱등키 `refund-{refund_id}` 는 롤백 후 재시도에서 재현 불가 | 6.2, 10.3 |
